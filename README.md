...
This project is for research, education, and exploratory analysis only. It is **not** investment advice, legal advice, accounting advice, or a recommendation to buy/sell any security.

Scores shown in the demo are generated by an LLM pipeline and may contain errors, bias, instability across runs, or outdated interpretations of source filings. They should be treated as machine-generated heuristics for discussion, not factual ratings.

For this repository's current demo run, company scores were produced by **Google Gemini** (default model in `score.py`: `gemini-2.5-flash-lite`, or the model specified via `GEMINI_MODEL` in `.env` at runtime). These model outputs do **not** represent the maintainer's personal opinions or discretionary judgments.
